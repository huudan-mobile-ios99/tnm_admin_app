...
import 'package:tnm_app_slot_aft/page/ranking/bloc/export.dart';
...
class RankingTopPageListBody extends StatefulWidget {
  const RankingTopPageListBody({super.key});

  @override
  State<RankingTopPageListBody> createState() => _RankingTopPageListBodyState();
}

class _RankingTopPageListBodyState extends State<RankingTopPageListBody> {
  ...
  final format_date = DateFormatter();
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ListSlotBloc, ListSLState>(
      builder: (context, state) {
        switch (state.status) {
          case ListSLStatus.failure:
            return Center(
                child: TextButton(
              onPressed: () {
                context.read<ListSlotBloc>().add(ListSlotFetch());
                // ignore: invalid_use_of_visible_for_testing_member
                context.read<ListSlotBloc>().emit( const ListSLState());
              },
              child: const Text('no ranking founds,press to retry'),
            ));
          case ListSLStatus.success:
                                            ...
                                            child: Row(
                                              ...
                                              children: [
                                                IconButton(
                                                    ...
                                                    onPressed: () {
                                                      debugPrint( 'update member topranking  ${state.posts[index].customerName}');
                                                      showDialog(
                                                          context: context,
                                                          builder:
                                                              (context) =>
                                                                      ...
                                                                      Row(
                                                                        ...
                                                                        children: [
                                                                          customButtonIcon(
                                                                            ...
                                                                            text:"SUBMIT",
                                                                            onTap: (){
                                                                              if(controllerMemberCurrent.text == controllerMemberNew.text || controllerMemberNew.text.isEmpty){
                                                                                showSnackBar(context:context,message:"Invalid fields");
                                                                              }else{
                                                                                service_api.updateRankingById(
                                                                                id: state.posts[index].id!,
                                                                                point: ( state.posts[index].point!).toDouble(),
                                                                                customer_number: controllerMemberNew.text,
                                                                              ).then((v){
                                                                                debugPrint('updateRankingById v: $v');
                                                                              }).whenComplete((){
  ...
  void _onScroll() {
    ...
    if (_isBottom) context.read<ListSlotBloc>().add(ListSlotFetch());
  }
